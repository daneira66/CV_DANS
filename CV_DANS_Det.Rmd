---
name: Daniel Alejandro
surname: Neira Sierra
position: "Psicólogo Especialista en Estadística Aplicada e Inteligenca Artificial"
address: "Bogotá D.C., Colombia"
phone: +57 3043553863
email: "daneira66@outlook.com"
github: daneira66
linkedin: Daniel-Neira-DANS
date: "`r format(Sys.time(), '%B %Y')`"
output: vitae::awesomecv
header-includes:
  - \usepackage{enumitem}
  - \setlist[itemize]{nosep, topsep=0pt, partopsep=0pt, leftmargin=*}
  - \geometry{left=1.5cm, top=1.5cm, right=1.5cm, bottom=2.0cm}
  - \patchcmd{\cventry}{\begin{cvitems}}{\vspace{0.5mm}\begin{cvitems}}{}{}
  - \patchcmd{\cventry}{\end{cvitems}}{\end{cvitems}\vspace{1mm}}{}{}
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE)
library(vitae);library(tibble)
```

# Educación

```{r}
tribble(
  ~ Degree, ~Advisor,~ Year, ~ Institution, ~ Where,
  "Especialización en Inteligencia Artificial", NA,"2026", "Fundación Universitaria Internacional de la Rioja", "Virtual, Colombia",
  "Especialización en Estadística Aplicada", "Tesis: Relación del deterioro cognitivo con el perfil lipídico en adultos mayores: Un análisis de correspondencias","2019", "Fundación Universitaria Los Libertadores", "Bogotá, Colombia",
  "Especialización en Estadística Aplicada", "Director: Heibar Yesid Rodríguez Pinzón","2019", "Fundación Universitaria Los Libertadores", "Bogotá, Colombia",
  "Psicología", "Tesis: Propiedades psicómetricas del Inventario de Trastornos de la Conducta Alimetaria (EDI-3) en adultos bogotanos - Fase 2","2012-2016", "Universidad Católica de Colombia", "Bogotá, Colombia",
  "Psicología", "Directora: Ivonne Alejo","2012-2016", "Universidad Católica de Colombia", "Bogotá, Colombia"
) %>% 
  detailed_entries(what=Degree, when=Year, with=Institution, where=Where,why = Advisor)
```

# Reconocimientos

```{r}
tribble(
  ~Year, ~Name, ~Institution,
  2016, "Grado de honor", "Facultad de psicología de la Universidad Católica de Colombia"
) %>% 
  detailed_entries(
    where = Year,
    with = Name,
    what = Institution
  )
```

# Experiencia

```{r}
tribble(
  ~Cargo, ~tiempo,~Institucion, ~Ciudad, ~Desc,
  "Profesional Especializado","04/11/2025 - Actualidad","Instituto Nacional de Medicina Legal y Ciencias Forenses","Bogotá, Colombia","Desarrollar herramientas, instrumentos y procedimientos de medición psicológica y de competencias laborales que se requieran en la Oficina de Personal para llevar a cabo los procedimientos, proyectos y actividades relacionadas con la Carrera Especial del Instituto, principalmente en lo concerniente a procesos de selección de ingreso y ascenso, evaluación de desempeño, estudios de seguridad, entre otros.",
  "Psicómetra de despacho","01/02/2025 - 29/10/2025","Comisión Nacional del Servicio Civil","Bogotá, Colombia","Prestar los servicios profesionales con un enfoque psicométrico, para apoyar etapas de planificación y ejecución de los Procesos de Selección, así como, apoyar la definición de la estructura de pruebas y ejes temáticos; además realizar el seguimiento, inspección y control del proceso en la elaboración, aplicación, análisis y calificación de las pruebas asignadas por el Despacho del Comisionado Edwin Arturo Ruiz Moreno.",
  "Psicólogo experto en psicometría","06/02/2025 - 10/10/2025","Instituto Nacional de Medicina Legal y Ciencias Forenses","Bogotá, Colombia","Prestar los servicios de profesionales para ejecutar las actividades técnicas y/o expertas requeridas para el diseño y validación de herramientas de evaluación psicométrica, la estructuración de pruebas para grupos funcionales y la planeación y ejecución de procesos de selección para desarrollar el sistema de carrera especial del INMLCF",
  "Investigador I", "9/10/2024 - 31/12/2024", "Universidad Nacional de Colombia","Bogotá, Colombia","Prestar los servicios profesionales especializados para llevar a cabo actividades relacionadas con los componentes del diagnóstico y propuesta de rediseño organizacional de la ADRES, a partir del análisis de su estructura organizacional, del modelo de operación por procesos y su recurso humano.",
  "Psicólogo experto en psicometría","22/02/2024 - 15/12/2024","Instituto Nacional de Medicina Legal y Ciencias Forenses","Bogotá, Colombia","Prestar los servicios de profesionales para ejecutar las actividades técnicas y/o expertas requeridas para el diseño y validación de herramientas de evaluación psicométrica, la estructuración de pruebas para grupos funcionales y la planeación y ejecución de procesos de selección para desarrollar el sistema de carrera especial del INMLCF",
  "Analista de datos","15/09/2024 - 25/10/2024","Colegio Colombiano de Psicólogos","Bogotá, Colombia","Realizar el análisis de datos y psicométrico para el proyecto PR11683357 COLPSIC – DINCO – EMBAJADA AMERICANA.",
  "Profesional de apoyo a pruebas y analista de datos","15/01/2023 - 18/12/2023","Fundación Universitaria del Área Andina","Bogotá, Colombia","Profesional de apoyo y analista de datos para el diseño, construcción, validación, análisis y calificación de las pruebas escritas y de ejecución para los procesos de selección de Entidades del Orden Territorial 2022, Entidades del Orden Nacional 2022 y DIAN 2022 ejecutadas para la Comisión Nacional del servicio Civil.",
  "Psicólogo","08/02/2023 - 08/01/2024","Secretaría de Educación Distrital","Bogotá, Colombia","Llevar a cabo el procesamiento de datos para la generación de información del Sistema Multidimensional de Evaluación para la Calidad Educativa - SMECE; formular y gestionar los indicadores que harán parte del SMECE; diseñar las historias de usuario para el desarrollo de las herramientas informáticas para la visualización del SMECE y hacer seguimiento al desarrollo e implementación de dichas herramientas.",
  "Psicólogo","16/08/2022 - 15/01/2023","Secretaría de Educación Distrital","Bogotá, Colombia","Llevar a cabo el procesamiento de datos para la generación de información del Sistema Multidimensional de Evaluación para la Calidad Educativa - SMECE; formular y gestionar los indicadores que harán parte del SMECE; diseñar las historias de usuario para el desarrollo de las herramientas informáticas para la visualización del SMECE y hacer seguimiento al desarrollo e implementación de dichas herramientas.",
  "Psicómetra de despacho","12/2020 - 02/2023","Comisión Nacional del Servicio Civil","Bogotá, Colombia","Planear y  diseñar las pruebas escritas y de ejecución para los procesos de selección DIAN y Aerocivil, lo cual implica la elaboración de las estructuras de prueba, el proceso de construcción y validación de ítems y el análisis y calificación de las pruebas. Además de apoyar en las etapas precontractules y contractuales para la ejecución de los procesos de selección por Instituciones de Educación superior, en lo concerniente a la construcción de documentos precontractuales en el apartado técnico y el apoyo a la supervisión en lo correspondiente a seguimiento y control de calidad de la construcción, validación, aplicación y calificación de las pruebas escritas y de ejecución.",
  "Psicómetra de ítems","04/2020","Fundación Universitaria del Área Andina","Bogotá, Colombia", "Realizar la revisión de los parámetros técnicos y metodológicos de los ítems construidos para las pruebas escritas del proceso de selección Territorial 2019; programar y llevar a cabo talleres de validación de ítems con los expertos temáticos; hacer seguimiento al cargue de los ítems en el aplicativo de construcción; emitir conceptos técnicos frente a las observaciones generadas por la Comisión Nacional del Servicio Civil sobre los ítems revisados.",
  "Psicólogo consultor","02/2020 - 4/2020","R.H.T. Soluciones y Diagnosticos Empresariales Ltda.","Bogotá, Colombia", "Elaborar ítems para tres (3) pruebas sobre los valores y comportamientos esperados para aspirantes a ingresar a la Policía Nacional, estudiantes de las Escuelas de instrucción y oficiales y suboficiales activos de la Policía Nacional; asistir a sesiones de capacitación y reuniones con la Entidad para entender el funcionamiento de los diferentes procesos; acatar y dar respuesta a las recomendaciones realizadas por los validadores de los ítems.",
  "Psicólogo de apoyo a la coordinación de pruebas","03/2019 - 12/2019","Universidad Francisco de Paula Santander","Bogotá, Colombia","Apoyar y coordinar el diseño y construcción de las pruebas básicas, funcionales y comportamentales para el proceso de selección 437 - Valle del Cauca desarrollado para la Comisión Nacional del Servicio Civil; realizar la revisión de los parámetros técnicos y metodológicos de los ítems construidos para las pruebas escritas del proceso de selección; programar y llevar a cabo talleres de validación de ítems con los expertos temáticos; hacer seguimiento al cargue de los ítems en el aplicativo de construcción; emitir conceptos técnicos frente a las observaciones generadas por la Comisión Nacional del Servicio Civil sobre los ítems revisados.",
  "Psicólogo de apoyo a la coordinación de pruebas","03/2019 - 12/2019","Universidad Francisco de Paula Santander","Bogotá, Colombia","Apoyar y coordinar lel diseño, construcción, validación, aplicación, análisis y calificación de las pruebas escritas de competencias básicas, funcionales y comportamentales del proceso de selección 437 - Valle del Cauca de la Comisión Nacional del Servicio Civil, realizando seguimiento y control de calidad para cada una de las actividades relacionadas con la etapa de pruebas escritas del proceso de selección.",
  "Experto constructor y/o validador de ítems","10/2019, 11/2019","Universidad Libre","Bogotá, Colombia", "Realizar la construcción y validación de ítems de las pruebas básicas, funcionales y comportamentales de la convocatoria Distrito Capital - CNSC desarrollada para la Comisión Nacional del Servicio Civil.",
    "Experto constructor y/o validador de ítems","10/2019, 11/2019","Universidad Libre","Bogotá, Colombia", "Realizar la construcción y validación de ítems de las pruebas básicas, funcionales y comportamentales de la convocatoria Territorial Norte desarrollada para la Comisión Nacional del Servicio Civil.",
  "Psicólogo constructor de ítems","05/2020 - 06/2020","Universidad Francisco de Paula Santander","Bogotá, Colombia", "Realizar la construcción y validación de ítems de las pruebas básicas, funcionales y comportamentales del proceso de selección 437 - Valle del Cauca de la Comisión Nacional del Servicio Civil.",
  "Psicólogo de apoyo","11/2017 - 01/2018, 03/2018 - 06/2018","Universidad Manuela Beltrán","Bogotá, Colombia", "Realizar la revisión de los parámetros técnicos y metodológicos de los ítems construidos para las pruebas escritas de los procesos de selección de la Agencia de Reincorporación Nacional - ARN, el  Instituto Geográfico Agustín Codazzi - IGAC y CAR - ANLA; hacer seguimiento al cargue de los ítems en el aplicativo de construcción; emitir conceptos técnicos frente a observaciones, quejas y/o reclamos que se realicen frente a los ítems; apoyar la elaboración de documentos y conceptos técnicos a entregar a la Comisión Nacional del Servicio Civil",
  "Psicólogo de apoyo","11/2017 - 01/2018, 03/2018 - 06/2018","Universidad Manuela Beltrán","Bogotá, Colombia", "Realizar la revisión de los parámetros técnicos y metodológicos de los ítems construidos para las pruebas escritas del proceso de selección CAR - ANLA; hacer seguimiento al cargue de los ítems en el aplicativo de construcción; emitir conceptos técnicos frente a observaciones, quejas y/o reclamos que se realicen frente a los ítems",
  "Constructor de ítems","09/2017, 11/2017", "Universidad Manuela Beltrán","Bogotá, Colombia","Construcción y validación de ítems para las pruebas básicas, funcionales y comportamentales de las pruebas básicas funcionales y comportamentales para la convocatoria 337 - IGAC desarrollada para la Comisión Nacional del Servicio Civil.",
    "Constructor de ítems","09/2017, 11/2017", "Universidad Manuela Beltrán","Bogotá, Colombia","Construcción y validación de ítems para las pruebas básicas, funcionales y comportamentales de las pruebas básicas funcionales y comportamentales para la convocatoria 338 - ACR desarrollada para la Comisión Nacional del Servicio Civil.",
  "Psicólogo clínico e investigador","07/2017 - 11/2018","Vida Resiliente SAS","Bogotá, Colombia","Diseño y aplicación de talleres psicoeducativos y terapias para niños, adolescentes y adultos",
  "Psicólogo clínico e investigador","07/2017 - 11/2018","Vida Resiliente SAS","Bogotá, Colombia","Diseño de instrumentos de evaluación psicológica, investigación cuantitativa y análisis de datos de los servicios prestados."
) %>%   detailed_entries(
    when = tiempo,
    where = Ciudad,
    what = Cargo,
    with = Institucion,
    why = Desc
  )
```

# Educación Complementaria

```{r}
tribble(
  ~ Curso, ~Duracion, ~ year, ~ Institucion, ~ Lugar,
  "Business Intelligence y Análisis de Big Data con Power Bi", "Duración: 80 horas", "10/11/2025 - 15/12/2025", "Corporación Instituto Colombiano de Cualificación Empresarial", "virtual, Colombia",
  "Diplomado en Habilidades Directivas y Liderazgo", "Duración: 96 horas", "02/12/2024 - 06/07/2025", "Fundación Universitaria Internacional de la Rioja", "Virtual, Colombia"
) %>% 
  detailed_entries(
    what = Curso, 
    when = year, 
    with = Institucion, 
    where = Lugar, 
    why = Duracion
  )
```

\newpage

# Habilidades

\begin{cvskills}
  \cvskill{Informática}{R, Python, Shiny, Power Bi, SPSS, Winsteps, Markdown}
  \cvskill{Análisis de datos}{Inferencia estadística, Análisis multivariado, Muestreo, Estadística no parámetrica, Modelos de regresión, Análisis de datos composicionales, Probabilidad, Análisis de inferencia causal, Visualización de datos, Metaanalisis}
  \cvskill{Psicometría}{Construcción de ítems, Diseño de pruebas, Análisis y calificación de pruebas con TCT, Análisis y calificación de pruebas con TRI}
  \cvskill{Inteligencia Artificial}{Modelos de aprendizaje supervisado, Modelos de aprendizaje no supervisado,Procesamiento de lenguaje natural, Procesamiento de imagenes, razonamiento automático}
  \cvskill{idiomas}{Español, Inglés}
\end{cvskills}


